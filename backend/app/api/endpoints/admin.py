...
async def get_analytics(current_user: User = Depends(auth_deps.get_current_user)):
    ...
    Requires: ADMIN or FUND_MANAGER role (OR logic)
    """
    return {
...
async def regional_features(
    ...
    """
    Access regional features.

    Requires: geography in ["US", "CA", "EU"]
    """
...
async def get_system_health(current_user: User = Depends(auth_deps.get_current_user)):
    """
...
@router.get("/profile")
async def get_user_profile(current_user: User = Depends(auth_deps.get_current_user)):
    """
    Get current user's profile with role and attribute information.

    No special permissions required - any authenticated user can access their profile.
    """
    return {
        "user_id": str(current_user.id),
