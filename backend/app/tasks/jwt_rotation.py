...
def _gather_current_key_set() -> KeySet:
    ...
    for kid, val in _config_service.JWT_KEYS.items():
    ...
    for kid in keys:
        if kid != _config_service.ACTIVE_JWT_KID:
            next_kid = kid
            break

    return KeySet(
        keys=keys,
        active_kid=_config_service.ACTIVE_JWT_KID,
        next_kid=next_kid,
        grace_period_seconds=_config_service.JWT_ROTATION_GRACE_PERIOD_SECONDS,
    )
...
def _apply_key_set_update(update: KeySetUpdate) -> None:
    ...
    if update.new_active_kid:
        old_kid = _config_service.ACTIVE_JWT_KID
        _config_service.ACTIVE_JWT_KID = update.new_active_kid
...
def _build_redis_client():  # pragma: no cover – isolation for patching
    ...
    redis_url = _config_service.REDIS_URL or "redis://localhost:6379/0"
    ...
    async def _run() -> None:  # noqa: D401 – nested coroutine
        ...
        try:
            async with acquire_lock(
                redis,
                "jwt_key_rotation",
                timeout=_config_service.JWT_ROTATION_LOCK_TTL_SEC,
            ) as got_lock:
                if not got_lock:
