...
async def test_authenticate_and_issue_tokens():
    ...
    usecase._OAuthUsecase__oauth_service.issue_tokens.assert_called_once_with(mock_user)
...
async def test_process_callback_success():
    ...
    usecase._OAuthUsecase__config_service.OAUTH_REDIRECT_URI = (
    ...
    with patch("app.usecase.oauth_usecase.verify_state", return_value=True):
        # Execute
        result = await usecase.process_callback(
            mock_request, "google", "code123", "state123", mock_redis
        )
...
async def test_process_callback_unsupported_provider():
    ...
    with pytest.raises(HTTPException) as excinfo:
        await usecase.process_callback(
            mock_request, "unsupported", "code123", "state123", mock_redis
        )
    assert excinfo.value.status_code == 404
...
async def test_process_callback_state_mismatch():
    ...
    with pytest.raises(HTTPException) as excinfo:
        await usecase.process_callback(
            mock_request, "google", "code123", "different_state456", mock_redis
        )
...
async def test_process_callback_invalid_state():
    ...
    with patch("app.usecase.oauth_usecase.verify_state", return_value=False):
        with pytest.raises(HTTPException) as excinfo:
            await usecase.process_callback(
                mock_request, "google", "code123", "invalid_state", mock_redis
            )
        assert excinfo.value.status_code == 400
...
async def test_exchange_code_google():
    ...
    mock_response.json.return_value = {
        "id_token": "eyJ0eXAiOiJKV1QiLCJhbGciOiJIUzI1NiJ9.eyJzdWIiOiIxMjMiLCJlbWFpbCI6InRlc3RAZXhhbXBsZS5jb20ifQ.fake_signature"
    }
    ...
    with patch("httpx.AsyncClient.post", return_value=mock_response), patch(
        ...
        result = await usecase._exchange_code(
            "google", "auth_code", "https://example.com/callback"
        )
...
async def test_exchange_code_unsupported_provider():
    ...
    with pytest.raises(ValueError) as excinfo:
        await usecase._exchange_code(
            "unsupported", "auth_code", "https://example.com/callback"
        )
    assert "Unsupported provider" in str(excinfo.value)
