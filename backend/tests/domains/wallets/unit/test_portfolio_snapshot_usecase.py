...
async def test_portfolio_snapshot_usecase_cache(portfolio_snapshot_usecase_with_di):
    ...
    portfolio_snapshot_usecase_with_di._PortfolioSnapshotUsecase__portfolio_snapshot_repo.get_by_wallet_address = AsyncMock(
        return_value=mock_data
    )

    # Call the method twice to test caching
    wallet_address = "0xabc"
    result1 = await portfolio_snapshot_usecase_with_di.get_snapshots_by_wallet(
        wallet_address
    )
    result2 = await portfolio_snapshot_usecase_with_di.get_snapshots_by_wallet(
        wallet_address
    )
    ...
    assert (
        portfolio_snapshot_usecase_with_di._PortfolioSnapshotUsecase__portfolio_snapshot_repo.get_by_wallet_address.call_count
        >= 1
    )
