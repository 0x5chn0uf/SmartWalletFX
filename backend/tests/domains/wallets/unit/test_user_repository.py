...
@pytest.mark.asyncio
async def test_user_repository_crud_and_exists(user_repository_with_real_db):
    """Full happy-path coverage for save, get_* and exists helpers."""
    # Use the real database fixture instead of mocked session
    ...
    assert by_username.username == user.username
    assert by_email.email == user.email
