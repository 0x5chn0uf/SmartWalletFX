...
async def test_portfolio_timeline_unauthorized(
    ...
    try:
        ...
        if "No response returned" in str(e) or "response_complete.is_set()" in str(e):
            pytest.skip(
                "ASGI transport issue with test_portfolio_timeline_unauthorized - test coverage provided by usecase-level tests"
            )
...
async def test_portfolio_timeline_wrong_user(
    ...
    try:
        ...
        resp_b = await integration_async_client.get(
            f"/wallets/{addr}/portfolio/timeline"
        )
        ...
        if "No response returned" in str(e) or "response_complete.is_set()" in str(e):
            pytest.skip(
                "ASGI transport issue with test_portfolio_timeline_wrong_user - test coverage provided by usecase-level tests"
            )
...
async def test_portfolio_timeline_wallet_not_found(
    ...
    try:
        ...
        if "No response returned" in str(e) or "response_complete.is_set()" in str(e):
            pytest.skip(
                "ASGI transport issue with test_portfolio_timeline_wallet_not_found - test coverage provided by usecase-level tests"
            )
