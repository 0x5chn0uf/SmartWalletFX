...
def init_database(db_path: Optional[str] = None) -> str:
    ...
    """
    from serena.database.migrations import auto_migrate
    ...
    if migrated:
        logger.info("Database initialized with migrations at %s", db_path)
    else:
        logger.debug("Database already up to date at %s", db_path)
