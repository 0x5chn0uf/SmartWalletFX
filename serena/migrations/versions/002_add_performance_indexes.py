...
def upgrade():
    ...
    op.execute("CREATE INDEX IF NOT EXISTS idx_archives_completed_at ON archives(completed_at DESC);")
    ...
    op.execute("CREATE INDEX IF NOT EXISTS idx_archives_kind_completed ON archives(kind, completed_at DESC);")
    
    # Index for embeddings task_id foreign key lookups
    op.execute("CREATE INDEX IF NOT EXISTS idx_embeddings_task_id ON embeddings(task_id);")
    
    # Index for embeddings chunk queries
    op.execute("CREATE INDEX IF NOT EXISTS idx_embeddings_task_chunk ON embeddings(task_id, chunk_id);")
    ...
    op.execute("CREATE INDEX IF NOT EXISTS idx_archives_filepath ON archives(filepath);")
    
    # Index for updated_at timestamp (for sync/change tracking)
    op.execute("CREATE INDEX IF NOT EXISTS idx_archives_updated_at ON archives(updated_at DESC);")
