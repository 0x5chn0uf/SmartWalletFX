...
def cmd_watch(args) -> None:
    ...
    from serena.infrastructure.indexer.indexer_core import MemoryIndexer
    
    try:
        # Initialize remote memory connection
        memory = RemoteMemory()
        if not memory.is_server_available():
            print("❌ Serena server is not running. Start it with: serena serve")
            sys.exit(1)
        
        print("✅ Connected to Serena server")
